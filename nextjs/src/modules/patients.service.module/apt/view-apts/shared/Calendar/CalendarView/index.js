...
const CalendarView = ({ weekDates, apts, setApts, selectedAptType, onClickApt }) => {
  ...
  useEffect(() => {
    ...
    apts.map(apt => {
      if (apt.aptState == 'ACTIVE') {
        let aptDate = moment(new Date(apt.aptDate + ' ' + apt.aptTime)).format("YYYY-MM-DD");
        if (weekDates.length && weekDates[0].isSameOrBefore(aptDate) && weekDates[weekDates.length - 1].isSameOrAfter(aptDate)) {
          arrNew.push(apt);
        }
      }
    });
      ...
      <div className="timeslot-space-wrapper" ref={spaceRef}>
        <div>
          {curWeekapts.map(apt => {
            ...
            if (selectedAptType != APT_TYPE_TO_SEARCH.ALL && apt.aptType != selectedAptType)
              return;
              
            let aptTime = moment(new Date(apt.aptDate + ' ' + apt.aptTime));
            let left = 26 + aptTime.diff(weekDates[0], 'day') * 120; //26 is the TIme visible left pixel
            let top = 0;
            for (let i = 0; i < aptTime.format("H"); i ++)
              top += timeslotHeights[i];
            top += aptTime.format("m") * 2;
            return (
              <div key={apt.id} style={{ position: "absolute", left: left, top: top, zIndex: 10 }}>                
                <AptCardSmall 
                  apt={apt} 
                  setApt={(v) => setApts(apts.map(u => u.id == v.id ? v : u))}
