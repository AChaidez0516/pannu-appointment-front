...
import TransactionResult from '../../../../user.module/payment/result'
...
import {
  ...
  Td,
  TdText,
} from '../../../../user.module/payment/shared/styled'
...
import {
  ...
  TopWrapper,
  TopText,
} from '../../../../user.module/payment/checkout/styled'
...
export default function StepThreeComponent(props) {
  ...
  const addCard = () => {
      router.push({ pathname: '/users/payment', query: {type: 'add_card', referrer: 'checkout'} })
  }
      ...
      { (!result) ? (
        <>
          ...
          <TextWrapper>
              ...
              <Text className="right">{dollarUSLocale.format(amountDue)}</Text>
          </TextWrapper>
          <TextWrapper>
              <Text className="left">Employer contribution</Text>
              <Text className="right">{dollarUSLocale.format(employerContribution)}</Text>
          </TextWrapper>
          <TextWrapper>
          ...
          <TableDiv>
              { /*<TrHeader className="justify-end" style={{ background: 'transparent', boxShadow: 'unset' }}>
                <Td>
                    <LinkButton className="smallest">Move</LinkButton>
                </Td>
            </TrHeader>*/ }
              <Tr className="header">
                  <Td style={{ width: '25%' }}>
              ...
              { paymentCardData&& paymentCardData.map((data, index) => {
                  return (
                    <Tr key={index} className={index == paymentCardData.length - 1 ? 'footer' : ''}>
                        <Td className="row align-center justify-between" style={{ width: '25%' }}>
                            { (checkCreditCard(data.cardNumber))&& <img src="/assets/images/bank-ico.png" width="22px" height="15px" /> }
                            <TdText>{data.nameOnCard}</TdText>
                        </Td>
                        <Td className="row align-center justify-center" style={{ width: '20%', margin: 5 }}>
              ...
              <Tr style={{ background: 'transparent', boxShadow: 'unset' }}>
                  <Td className="align-end" style={{ width: '100%' }}>
                      <LinkButton onClick={addCard} className="small">Add another card</LinkButton>
                  </Td>
              </Tr>
          </TableDiv>
          ...
          <div style={{ marginTop: 20 }}>
              ...
              <TextWrapper>
                  ...
                  <Text className="right"><span style={{ border: '1px solid #ccc', padding: 5, borderRadius: 5 }}>{getBalance()}</span></Text>
              </TextWrapper>
          </div>

          <CenteredRow marginTop={50}>
              <CustomButton onSuccess={() => done()} />
          </CenteredRow>
        </>
      ) : (
        <TransactionResult transactions={transactions} referrer={referrer} />
      ) }
    </StepWrapperComponent>
  )
}
