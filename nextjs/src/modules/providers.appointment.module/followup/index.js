...
import {
  ...
  HistoryWrapper, FollowUpWrapper
} from "./styled";
import {
  CalendarIcon, CircleCheckIcon, ClockIcon
...
import {
  ...
  numberOptions,
  periodItems,
  periodOptions, priorityItems, priorityOptions,
  virtualVisitItems,
  virtualVisitOptions
} from "./options";
...
import { fakePreparations, fakeInstructions, fakePreviousHistory, fakeReasons } from './data';
import {
...
function FollowUpAppointment({
  ...
  useEffect(() => {
    if (!isMock) {
      const checkIfIncluded = (allItems, someItems) => {
        ...
        return allItems.map(a => someItems.find(s => s?.id === a?.id) ? ({ ...a, checked: true }) : a)
      }
      const fetchPreparationInstructionReasonsByProviderId = async () => {
        try {
          const providerId = aptDetails.providerUserId || 16;
          const res = await getPreparationsAndInstructionsAndReasons(providerId)
  ...
  const handleOptionChange = (event) => {
    setFormData({ ...formData, isNormalDate: event.target.value === 'datetime' })
  };
  ...
  const controlProps = (item) => ({
    checked: item === 'datetime' ? formData.isNormalDate : !formData.isNormalDate,
  ...
  const handleConfirmPreparation = () => {
    handleConfirmAdd()
    setNewPreparations(newPreparations.map(p => _preparationIds.includes(p.id) ? ({ ...p, checked: true }) : ({ ...p, checked: false })))
  }

  const handleConfirmInstruction = () => {
    handleConfirmAdd()
    setNewInstructions(newInstructions.map(p => _instructionIds.includes(p.id) ? ({ ...p, checked: true }) : ({ ...p, checked: false })))
  }

  const handleConfirmReason = () => {
    setNewReasons(newReasons.map(p => _reasonIds.includes(p.id) ? ({ ...p, checked: true }) : ({ ...p, checked: false })))
  }
  ...
  const handleAddPreparation = (title, text) => {
    setNewPreparations([...newPreparations, { id: uuidv4(), title: title || 'Preparation', description: text, checked: addNewItemCheck }])
    initEditor()
  }

  const handleAddInstruction = (title, text) => {
    setNewInstructions([...newInstructions, { id: uuidv4(), title: title || 'Instruction', description: text, checked: addNewItemCheck }])
    initEditor()
  }
    ...
    <div className="section">
      <FollowUpWrapper>
        <>
          ...
          <Row1>
            <DoubleCheckItem desc="Preparations" showCheck={newPreparations.some(v => v.checked)} handleClick={() => {
            ...
            <DoubleCheckItem desc="Instructions" showCheck={newInstructions.some(v => v.checked)} handleClick={() => {
          ...
          <Row2>
            <div className="d-flex">
              <div>
                <Radio
                  ...
                  icon={<RadioOffIcon />}
                  {...controlProps("period")} sx={{ padding: '5px' }}
                />
              </div>
              <div className="d-flex gap1">
                ...
                <SelectPopup
                  ...
                  onChange={(id) => setFormData({ ...formData, aptWMDNumber: id })}
            ...
            <div className="d-flex">
              <Radio
                checkedIcon={<RadioOnIcon />}
                icon={<RadioOffIcon />}
                {...controlProps("datetime")} sx={{ padding: '5px' }}
              />
              <div className="d-flex gap1">
                <InputBox
                  caption="Date"
          ...
          <Row4>
            <div className="d-flex">
              ...
              <Checkbox
                ...
                onChange={() => setFormData({ ...formData, cantBeChanged: !formData?.cantBeChanged })}
              />
            </div>
            <div className="d-flex">
          ...
          <Row5>
            ...
            <div className="edit" onClick={() => { setShowReasonsModal(true); _setReasonIds([]) }} >Edit</div>
            <DescriptionComponent className="line" description={mergeReasons(newReasons.filter(r => r.checked))} onClickMore={() => onClickMore(mergeReasons(newReasons.filter(r => r.checked)))} maxLength={200} />
          </Row5>
          <Row6>
            <div className="row-section1">
              <span className="desc">If appointment is <span className="bold">rescheduled</span></span>
              <div className="d-flex">
                <div className="double-check-item">
                  Refill <br />medicines
                  {false && (
                    <span className="check-box"><CircleCheckIcon /></span>
                  )}
                </div>
                <div className="double-check-item">
                  Reorder<br /> tests
            ...
            <div className="double-check-item" onClick={() => setEditBox({ type: EDITOR_TYPE.MY_NOTES, show: true })}>
              My Note
              {false && (
                <span className="check-box"><CircleCheckIcon /></span>
              )}
          ...
          <Box mt={60} px={9}>
            <ActionBtnWrapper twoMoreBtns>
              <ActionBtn fontS={22} lineH={12} onClick={() => goToDetails(aptDetails.id)}>Cancel</ActionBtn>
              {true && <ActionBtn fontS={22} lineH={12} onClick={handleDone}>Done</ActionBtn>}
              {false && <ActionBtn fontS={22} lineH={12} onClick={handleGoToBilling}>Go to billing</ActionBtn>}
            </ActionBtnWrapper>
          </Box>
        </>
...
const CheckedOnIcon = () => {
  return (
    <Image src={ICONS.checkOn} width={15} height={15} layout="fixed" />
  )
}

const CheckedOffIcon = () => {
  return (
    <Image src={ICONS.checkOff} width={15} height={15} layout="fixed" />
  )
}

const RadioOnIcon = () => {
  return (
    <Image src={ICONS.radioOn} width={24} height={24} layout="fixed" />
  )
}

const RadioOffIcon = () => {
  return (
    <Image src={ICONS.radioOff} width={24} height={24} layout="fixed" />
  )
}
